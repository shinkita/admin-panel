...
class Deleted_users extends CI_Controller {
    ...
    public function index() {        
        ...
        $this->load->view('dusers', $this->data);
        $this->load->view('app-footer', $this->data);
    }   
    public function profile($id)
    {
        $data = array(
            'where'=>array('b_users.user_id'=>$id),
            'select'=>'*',
            'join'=>array(array('table'=>'user_detail','on'=>'b_users.user_id=user_detail.user_id','type'=>'LEFT'),array('table'=>'(SELECT * FROM `login_history` ORDER BY `date_time` DESC) as login_history','on'=>'b_users.user_id=login_history.user_id','type'=>'LEFT'))
        );
        $this->data['user_id']= $id;
        $this->data['user_profile'] = $this->usersmodel->dget_row($data);
    ...
    public function events($id,$page=0){
      ...
      $count_photo = $this->usersmodel->fetch_query("SELECT count(event.id) AS COUNT FROM event LEFT JOIN post ON post.event=event.id WHERE event.user_id='$id' ".$query_extend."");      
      $this->data['user_id']= $id;  
        $this->data['count_rows'] = $count_photo[0]['COUNT'];
        ...
        $config['full_tag_open'] = '<div class="my-pagination">';
        $config['full_tag_close'] = '</div>';
        $query = "SELECT *,event.id AS event_id, event.deleted AS deleted FROM event LEFT JOIN post ON post.event=event.id WHERE event.user_id='$id' ".$query_extend." ORDER BY event.id DESC LIMIT $page, $config[per_page]";
        $this->data['events'] = $this->usersmodel->fetch_query($query); 
        if(count($this->data['events']>0))
        {
            foreach ($this->data['events'] as $key => $value) {
                $query = "SELECT event_attand.user_id AS user_id, profile_pic.profile_pic AS profile_pic, user_detail.name AS name FROM event_attand JOIN user_detail ON user_detail.user_id=event_attand.user_id JOIN profile_pic ON profile_pic.user_id=event_attand.user_id AND profile_pic.profile_type='".$value['profile_type']."' WHERE post_id='".$value['id']."'";
    ...
    public function profile_photo($page=0)
    {
      $query_extend = '';        
        ...
        $config['full_tag_open'] = '<div class="my-pagination">';
        $config['full_tag_close'] = '</div>';                
        $query = "SELECT post.id as id, images, post.date_time, profile_type, method, post.deleted, user_detail.name FROM post LEFT JOIN user_detail ON post.user_id=user_detail.user_id WHERE post_type = '2' ".$query_extend." ORDER BY post.id DESC LIMIT $page, $config[per_page]";
        // $config['use_page_numbers'] = TRUE;
        $this->data['photos'] = $this->usersmodel->fetch_query($query);
        $this->pagination->initialize($config);
        $this->data['paging'] = $this->pagination->create_links();
    ...
    {           
        ...
        $config['full_tag_open'] = '<div class="my-pagination">';
        $config['full_tag_close'] = '</div>';
        $query = "SELECT *,post.id as id,post.deleted AS deleted FROM post LEFT JOIN user_detail ON post.user_id=user_detail.user_id WHERE post_type=3 ".$query_extend." ORDER BY post.id DESC LIMIT $page, $config[per_page]";
        $this->data['videos'] = $this->usersmodel->fetch_query($query);   
        $this->pagination->initialize($config);
        $this->data['paging'] = $this->pagination->create_links();    
        $this->load->view('app-header', $this->data);
    ...
    public function profile_events($page=0){      
      $this->data['active_list'] = 'Profile_pages';
      $count_photo = $this->usersmodel->fetch_query("SELECT count(id) AS COUNT FROM event");            
        $this->data['count_rows'] = $count_photo[0]['COUNT'];
        ...
        $config['full_tag_open'] = '<div class="my-pagination">';
        $config['full_tag_close'] = '</div>';
        $query = "SELECT *,event.id AS event_id,post.id AS id,event.deleted AS deleted FROM event LEFT JOIN post ON post.event=event.id LEFT JOIN user_detail ON post.user_id=user_detail.user_id ORDER BY event.id DESC LIMIT $page, $config[per_page]";
        $this->data['events'] = $this->usersmodel->fetch_query($query); 
        if(count($this->data['events']>0))
        {
            foreach ($this->data['events'] as $key => $value) {
                $query = "SELECT event_attand.user_id AS user_id,user_detail.name AS name, profile_pic.profile_pic AS profile_pic FROM event_attand JOIN user_detail ON user_detail.user_id=event_attand.user_id JOIN profile_pic ON profile_pic.user_id=event_attand.user_id AND profile_pic.profile_type='".$value['profile_type']."' WHERE post_id='".$value['id']."'";
                $this->data['events'][$key]['user_detail'] = $this->usersmodel->fetch_query($query);
            }
        }
        $this->pagination->initialize($config);
    ...
    public function deactive_event($id)
    {
        ...
        {
            $query = "UPDATE `event` SET `deleted` = '1' WHERE `event`.`id` ='".$id."'";
            $response = $this->usersmodel->update_query($query);
